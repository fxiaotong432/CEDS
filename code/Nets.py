...
class VNet_dual_atten_CBAM(nn.Module):
    """
    ...
    def __init__(
        ...
        self.down_tr256 = DownTransition(spatial_dims, 128, 2, act, dropout_prob=dropout_prob_down, bias=bias)
        # The first decoder
        self.de1_up_tr256 = UpTransition(spatial_dims, 256, 256, 2, act, dropout_prob=dropout_prob_up)
        self.de1_up_tr128 = UpTransition(spatial_dims, 256, 128, 2, act, dropout_prob=dropout_prob_up)
        self.de1_up_tr64 = UpTransition(spatial_dims, 128, 64, 1, act)
        self.de1_up_tr32 = UpTransition(spatial_dims, 64, 32, 1, act)
        self.de1_out_tr = OutputTransition(spatial_dims, 32, out_channels, act, bias=bias)
        # The second decoder
        self.de2_up_tr256 = UpTransition(spatial_dims, 256, 256, 2, act, dropout_prob=dropout_prob_up)
        self.de2_up_tr128 = UpTransition(spatial_dims, 256, 128, 2, act, dropout_prob=dropout_prob_up)
        self.de2_up_tr64 = UpTransition(spatial_dims, 128, 64, 1, act)
        self.de2_up_tr32 = UpTransition(spatial_dims, 64, 32, 1, act)
        self.de2_out_tr = OutputTransition(spatial_dims, 32, out_channels, act, bias=bias)
        self.de2_confi = OutputTransition(spatial_dims, 5, out_channels, act, bias=bias)
        self.cbam_block = CBAMBlock3D(channel=5,reduction=2)
    ...
    def forward(self, x, need_fp=False):
        ...
        if need_fp:
            features = nn.Dropout3d(0.5)(features)

        x1 = self.de1_up_tr256(features, out128)
        x1 = self.de1_up_tr128(x1, out64)
        x1 = self.de1_up_tr64(x1, out32)
        x1 = self.de1_up_tr32(x1, out16)
        prediction = self.de1_out_tr(x1)
        prob = F.softmax(prediction, dim=1)
        uncertainty = -torch.sum(prob * torch.log(prob + 1e-6), dim=1, keepdim=True)
        x2 = self.de2_up_tr256(out256, out128)
        x2 = self.de2_up_tr128(x2, out64)
        x2 = self.de2_up_tr64(x2, out32)
        x2 = self.de2_up_tr32(x2, out16)
        x2 = self.de2_out_tr(x2)
        concated_output = torch.cat([x2, prediction, uncertainty], dim=1)
        confi = self.cbam_block(concated_output)
        confi = self.de2_confi(confi)
        return prediction, confi
    
    
class VNet_dual_noatten(nn.Module):
    """
    ...
    def forward(self, x):
        ...
        # Prediction
        x1 = self.de1_up_tr256(out256, out128)
        x1 = self.de1_up_tr128(x1, out64)
        x1 = self.de1_up_tr64(x1, out32)
        x1 = self.de1_up_tr32(x1, out16)
        prediction = self.de1_out_tr(x1)
